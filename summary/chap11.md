...
---
## 위임을 상속으로 전환 Replace Delegation with Inheritance
- 위임을 이용중인데 인터페이스 전반에 간단한 위임으로 도배하게 될 땐 위임 클래스를 대기 객체의 하위클래스로 만들자.

### 동기
- 주의사항1. 위임하려는 클래스의 모든 메서드를 사용하는 게 아닐 경우엔 적용하면 안된다.
  - 하위클래스는 반드시 상위클래스의 인터페이스를 따라야 하기 때문
- 주의사항2. 대리 객체를 둘 이상의 객체가 사용하고 변경 가능할 때
  - 데이터를 더 이상 공유할 일이 없어서 위임을 상속으로 바꿀 수 없다.

### 방법
- 위임클래스를 대리 객체의 하위클래스로 만들자
- 컴파일
- 대리 필드에 개리 객체 자체를 할당
- 단순 위임 메서드를 전부 삭제
- 컴파일, 테스트
